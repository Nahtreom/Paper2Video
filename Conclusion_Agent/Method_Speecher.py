...
def main():
    ...
    # 设置文件路径
    current_dir = os.path.dirname(os.path.abspath(__file__))
    prompt_template_path = os.path.join(current_dir, "prompt_templates", "Speecher.txt")
    
    # 如果没有提供上一个页面的讲稿路径，使用默认文件
    if args.previous_speech_path is None:
        previous_speech_path = os.path.join(current_dir, "prompt_templates", "Speecher-1.txt")
