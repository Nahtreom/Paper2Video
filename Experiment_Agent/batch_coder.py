...
def process_markdown_folder(input_folder: str, output_dir: str):
    ...
    print(f"⏱️  预估总时长: {total_files * 60}~{total_files * 90} 秒 (每个文件约60-90秒)")
    print("\n📋 处理顺序：")
    ...
    for index, markdown_file in enumerate(markdown_files, 1):
        ...
        try:
            ...
            success_count += 1
            print(f"✅ 完成！耗时: {format_duration(file_duration)}")
            
        except subprocess.CalledProcessError as e:
            file_duration = time.time() - file_start_time
            failed_files.append(relative_path)
            print(f"❌ 处理失败 (耗时: {format_duration(file_duration)})")
            print(f"   错误信息: {str(e)}")
            continue
    ...
    # 打印总结报告
    print_separator()
    print(f"\n🎯 代码生成完成! 成功生成 {success_count} 个Python代码文件")
    print_separator("-")
    print(f"📊 详细统计:")
