...
def copy_images_directory(images_dir, target_dirs):
    ...
    for target_dir in target_dirs:
        try:
            ...
            print(f"   ❌ 复制到 {target_dir} 失败: {str(e)}")
            failed_count += 1
    
    print(f"📷 图片目录复制完成: 成功 {copied_count} 个，失败 {failed_count} 个")
...
def step2_physical_split(segmentation_dir, split_pages_dir):
    ...
    print(f"📄 找到分割文件: {segmentation_file}")
    ...
    print(f"🎯 物理分割完成! 成功生成 {len(page_files)} 个页面文件")
...
def step3_batch_coding(split_pages_dir, generated_code_dir):
    ...
    print(f"🎯 代码生成完成! 成功生成 {len(code_files)} 个Python代码文件")
...
def step4_batch_speech(split_pages_dir, generated_code_dir, generated_speech_dir):
    ...
    print("🔄 开始批量演讲稿生成，将实时显示每个文件对的处理进度...")
    ...
    print(f"🎯 演讲稿生成完成! 成功生成 {len(speech_files)} 个演讲稿文件")
    ...
    while True:
        try:
            ...
            if len(matched_files) == 1:
                ...
                print(f"📂 打开 {file_type}: {filename}")
                print(f"   路径: {file_path}")
                
                # 使用vim打开文件
                try:
                    subprocess.run(['vim', file_path], check=True)
                    print(f"✅ 文件编辑完成: {filename}")
                except subprocess.CalledProcessError as e:
                    print(f"❌ 打开文件失败: {e}")
                except FileNotFoundError:
                    print("❌ 未找到vim编辑器，请确保vim已安装")
                    print("💡 您也可以使用其他编辑器手动打开文件:")
                    print(f"   {file_path}")
...
def print_final_summary(dirs, paper_path, start_time):
    ...
    try:
        ...
        if page_count == code_count == speech_count:
            print(f"   ✅ 所有文件生成完整，页面匹配完美!")
        else:
            print(f"   ⚠️  文件数量不匹配，请检查生成结果")
    except Exception as e:
        print(f"   统计文件数量时出错: {e}")
    ...
    try:
        ...
        if skip_interactive:
            ...
            if generated_files:
                ...
                print("⚠️ 未找到生成的文件，跳过交互式编辑")
        
        print("\n🎉 Pipeline 全部完成！感谢使用 Experiment_Agent！")
